...
export class StatusbarUi {
  ...
  static working(status: ServerStatus = ServerStatus.START) {
    ...
    StatusbarUi.log(`Server ${status}ing...`);
  }
  ...
  static stopServer(status: ServerStatus.START | ServerStatus.RESTART = ServerStatus.START, url: string) {
    setTimeout(() => {
      StatusbarUi.statusBarItem.text = `$(circle-slash) Port : ${Settings.port}`;
      StatusbarUi.statusBarItem.command = Commands.STOP_SERVER;
      StatusbarUi.statusBarItem.tooltip = 'Click to stop mock server';
      
      const statusMsg = `Server ${status}ed at port : [${Settings.port}](${url})`;
      Prompt.showPopupMessage(statusMsg);
      StatusbarUi.log(`[Done] Server ${status}ed at port: ${Settings.port}`);
    }, 150);
  }
}
